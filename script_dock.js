...
(function () {
  ...
  function open(id) {
    ...
    /* 보고 있는 동안에는 표시를 지웁니다.
       ★ ⏱️ 뽀모방만 빼요 — 거기 배지는 "안 읽은 글" 이 아니라 **인원**이라,
         내가 들어가 있다고 지우면 안 됩니다 (3명인데 0으로 바뀌어 버려요). */
    if (id !== "proom") badge(id, 0);
    dot(id, false);
  ...
  function close(id, 나가기) {
    ...
    /* 🎧 [2026-09-21 — 콩] 게시판도 같습니다. 여태 한 번 열면 그 세션 내내
       듣고 있었어요 — 끊겼다 붙을 때마다 그 게시판을 통째로 다시 받았습니다.
       특히 🏢 품평은 **안 시드는** 자리라 계속 자랍니다.
       ★ 비밀방이 이미 이렇게 하고 있었어요. 같은 길을 씁니다. */
    if (pid === "pub")  window.closePubReview?.();
    if (pid === "help") window.closeHelp?.();
    if (pid === "qna")  window.closeQna?.();
