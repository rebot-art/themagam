...
(function () {
  ...
  /* =====================================================================
     🧘 [2026-08-22 — 콩] 판도 제 크기를 지킵니다 (혼자 방 먼저)
     ---------------------------------------------------------------------
     styles.css 에 이 한 줄이 있습니다:
         html[data-solo][data-unzoom] .dock-panel{ zoom: var(--unzoom, 1); }
     그러면 판의 **겹친 배율이 정확히 1** 이 됩니다 — 화면은 70% 인데
     판만 제 크기예요.

     ★★★ 여기가 핵심입니다. 그 순간 uiZoom() 은 **판에게는 틀린 자**가
       됩니다. 판의 style.left · offsetWidth 는 이제 화면 값과 1:1 이거든요.
       자를 안 갈면 끌 때 커서를 안 따라가고 화면 끝에서 먼저 막힙니다
       (예전에 똑같이 겪었습니다 — "재는 자가 둘").

     ★ 아래 조건은 **CSS 의 선택자와 글자 그대로 같아야 합니다.**
       한쪽만 고치면 자가 어긋나요. checks.js 가 둘을 맞춰 봅니다.
     ===================================================================== */
  const 판제크기 = () => {
    const h = document.documentElement;
    return h.hasAttribute("data-solo") && h.hasAttribute("data-unzoom");
  };
  const Z = () => (판제크기() ? 1 : (window.uiZoom?.() || 1));
  window.dockZoom = Z;      // 딴 파일이 판 좌표를 잴 일이 생기면 이걸 쓰세요
