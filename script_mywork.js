...
(function () {
  ...
  function ruleInfo(y, m, attended, vacCount) {
    ...
    /* 🎑 진행바 아래 안내에 쓸 **기본 기준** — 휴가를 하나도 안 찍었을 때
       (2026-09-13 — 콩 "휴가 체크 안 했을 때의 기본으로 명시해줘").
       휴가까지 섞으면 사람마다 16→14, 15→13 … 으로 제각각이라
    ...
    /* =====================================================================
       ...
       ① **오늘을 안 셌습니다** (k > today). 오늘은 아직 안 끝난 날이라
          지금 들어오면 출석이 됩니다. 이미 오늘 출석했으면 그건 attended
          에 들어 있으니, **아직 안 한 날만** 한 칸으로 셉니다.

       ② **연휴를 뺐습니다.** 휴가와 같은 취급을 했는데, 둘은 다릅니다 —
            🏖️ 휴가  : 내가 "안 나온다" 고 **선언한** 날
            🎑 연휴  : 기준은 낮아지지만 **나오면 그대로 출석**인 날
          연휴에 쓰는 사람이 실제로 있는데 그 날을 "나올 수 없는 날" 로
          세니, 채울 수 있는 사람에게 🔴 불가가 떴습니다.
          기준을 두 번 깎는 게 아니냐 — 아닙니다. 기준(need)에서 뺀 것은
          **의무**를 줄인 것이고, 여기서 세는 것은 **기회**예요. 연휴는
          의무는 아니지만 기회이긴 합니다.
       ===================================================================== */
    ...
    if (이번달인가) {
      for (let d = 1; d <= daysInMonth; d++) {
        ...
        if (k < today) continue;                       // 지나간 날
        if (k === today && _days[k]) continue;         // 오늘 이미 출석 — attended 에 있음
        if (_vacs[k] === true) continue;               // 안 나온다고 한 날
        daysLeft++;
      }
    }
  ...
  function ruleHtml(y, m, attended, vacCount) {
    ...
    /* 🎑 연휴로 기준이 내려간 달 — 진행바 **바로 아래**에 한 줄로 못 박습니다
       (2026-09-13 — 콩 "한눈에 보이게").
