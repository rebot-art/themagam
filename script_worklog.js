...
(function () {
  ...
  document.addEventListener("click", async (e) => {
    ...
    /* ★ [2026-08-28 — 콩] 지우기 전에 한 번 묻습니다.
       ---------------------------------------------------------------
       [무슨 일이 있었나]
       콩이 프롤로그를 마침 처리했는데 줄이 목록에 그대로 남길래
       ✕ 로 치웠습니다. 그런데 작품 탭에서도 함께 사라졌어요.
       ★ 작품 탭은 따로 쌓아 둔 기록이 아니라 **이 줄들을 그 자리에서
         세는** 화면입니다 (작품회차() 참고). 줄이 사라지면 편수·
         편당 평균·모두 N자·진도 막대가 같이 빠집니다.
       ★ 되살릴 수 없으니, 무엇을 잃는지 회차와 글자수까지 보여 줍니다.
       ★ 작품 지우기(delwork)·할 일 지우기(script_data.js)에는 진작
         확인창이 있었는데 여기만 없었습니다. 이제 셋 다 묻습니다. */
    if (act === "del") {
      const r = W.회차들()[id];
      if (r) {
        const 이름 = W.작품(r.w)?.name;
        const 앞 = (이름 ? 이름 + " · " : "") + 회차글(r.w, r.ep);
        const 자 = Number(r.cnt) || 0;
        if (!confirm(`${앞} 줄을 지웁니다.\n\n` +
                     (자 ? `적어 두신 ${자.toLocaleString("ko-KR")}자가 함께 사라져요 — ` : "") +
                     "작품 탭의 편수·평균·진도도 그만큼 빠집니다.\n" +
                     "되살릴 수 없어요.\n\n" +
                     "※ 오늘 쓴 글자수와 업적은 따로 쌓이니 줄지 않습니다.")) return;
      }
