...
/* =====================================================================
   ...
   ---------------------------------------------------------------------
   [무엇인가]
   비밀방·수다방·뽀모방의 **어제까지의 대화를 지웁니다.** 이 세 방의
   대화는 그날의 수다일 뿐이라 쌓아 둘 이유가 없어요 (콩 확정).
   메인 Chat 은 그대로 둡니다 — 지난 발언을 지우지 않는 방침 유지.

   [어떻게 — 서버 없이, 각자에 얹혀서]
   파이어베이스 무료판에는 시간 맞춰 도는 서버가 없습니다. 그래서
   **그날 처음 방을 여는(또는 자정을 넘기는) 사람의 기기가** 청소를
   해요. 눈 청소와 같은 이치 — 먼저 나온 사람이 쓸면 끝입니다.
     · 기기마다 하루 한 번(도장 = 오늘 날짜, AppStore)
     · 여럿이 동시에 쓸어도 지우기는 겹쳐도 무해합니다 (이미 없는 걸
       또 지우면 아무 일도 안 일어나요)
     · 400줄씩 최대 20바퀴 — 밀린 옛 대화도 며칠 안에 다 빠집니다

   [권한 — 보안규칙을 안 바꿔도 됩니다]
   sroom·proom·messages2 의 $id 쓰기가 이미 "auth != null"(비밀방은
   승인 멤버)이고, **지우기는 newData 가 없어 .validate 를 안 탑니다.**
   방에 들어올 수 있는 사람이면 누구나 쓸 수 있어요.

   [날짜의 기준]
   기기의 로컬 자정입니다 — 글자수 롤오버와 같은 철학("내가 보는
   날짜"). 멤버가 전부 한국이라 실질적으로 같은 순간이에요.

   ★ 혼자 방(?solo=1)은 건너뜁니다 — 가짜 DB 가 orderByChild 쿼리를
     다 흉내 내지 못하고, 거긴 메모장이라 지울 이유도 없어요.
   ===================================================================== */
(function () {
  "use strict";

  async function 자정방청소(경로, 도장키) {
    try {
      if (!window.db || window.SOLO) return;
      const 오늘 = new Date(); 오늘.setHours(0, 0, 0, 0);
      const 도장 = 오늘.getFullYear() + "-" + (오늘.getMonth() + 1) + "-" + 오늘.getDate();
      /* 하루 한 번 — 대화가 올 때마다 불려도 여기서 곧장 돌아갑니다 */
      if (window.AppStore?.getItem(도장키) === 도장) return;
      try { window.AppStore?.setItem(도장키, 도장); } catch (e) {}
      /* ★ 도장을 먼저 찍습니다 — 실패하면 이 기기는 오늘 다시 안 쓸지만,
         다른 기기들이 저마다 쓸어 주니 빠뜨릴 일은 없습니다 */
