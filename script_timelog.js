...
(function () {
  ...
  /* =====================================================================
     ...
     **다섯 벌로 복사**돼 있었어요 — 내 기록 화면, 카드에 뜨는 ⏱,
     ...
     [무엇을 얼마나 인정하나 — 2026-08-23 운영진 회의 확정]
         ...
         🛠️REPAIR   30%   방을 손보는 중 (2026-09-08 — 콩 "0은 아쉬워 ㅋㅋ")

     한 줄로: **집필만 온전히 인정하고, JOB·multiT 는 70%, 방 손보기는 30%.**
     ★ REPAIR 는 원래 0% 였습니다 — 글쓰기가 아니니까요. 다만 방을 고치는
       동안에도 그 자리에 앉아 있는 것은 맞고, 그 시간이 통째로 사라지면
       손보는 사람만 손해라 30% 로 조금 얹었습니다. 다른 상태와 달리
       **방장·운영진만 고르는 상태**라 남과의 형평 문제도 크지 않아요.
  ...
  async function commitSegs(updates, cur) {
    ...
    /* 📊 방이 함께 보는 자리에도 얹습니다 (2026-08-30 — 콩) */
    공개시간올리기(updates);
  }

  /* =====================================================================
     📊 worktime — 방이 함께 보는 작업 시간 (2026-08-30 — 콩)
     ---------------------------------------------------------------------
     [왜 따로 두는가]
     원본 구간은 `users/{닉}/timeSegs` 에 있는데, 그 자리는 **본인과
     방장만** 읽습니다 — 옆에 프로필·할 일이 함께 살아서 열 수가 없어요.
     그래서 **합산된 숫자만** 공개 칸에 따로 적습니다.
         worktime/{날}/{닉} = 그날 작업 분(分)
     글자수(wordlog)와 같은 결이에요 — 자세한 건 잠근 채, 자랑할 숫자만.

     [분 단위인 까닭] ms 로 적으면 구간이 바뀔 때마다 값이 달라져 쓰기가
     잦아집니다. 분으로 반올림하면 대개 그대로라 **안 바뀌면 안 씁니다**
     (status 다이어트에서 쓴 그 수법).

     ★ 이 파일은 원본을 먼저 적고 **그 다음에** 여기를 적습니다. 어느
       한쪽이 실패해도 원본이 진실이에요 — 표가 조금 어긋나면 다음
       구간이 저장될 때 저절로 맞습니다.
  ...
  async function 공개시간올리기(updates) {
    if (!myNick || !db) return;
    /* 이번에 손댄 날짜들만 다시 셈합니다 — 대개 하루, 자정 걸치면 이틀 */
    const 날들 = new Set();
    Object.keys(updates || {}).forEach((k) => {
      const m = /^timeSegs\/(\d{4}-\d{2}-\d{2})\//.exec(k);
      if (m) 날들.add(m[1]);
    });
    if (!날들.size) return;
    for (const day of 날들) {
