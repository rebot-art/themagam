...
(function () {
  ...
  /* =====================================================================
     ...
     [★★★ 공유 중이어도 접속 인원이 넘으면 꺼집니다 — 고침 2026-09-21 (콩)]
     처음에는 "이미 켠 사람은 그대로" 였습니다. 그런데 콩이 짚었어요 —
     **그러면 미리 켜 두는 것이 이득**이 됩니다. 한산할 때 얼른 켜 두고,
     사람이 몰려도 혼자 계속 틀어 두게 돼요. 막으려던 바로 그 시간대에
     제일 무거워지는 셈이라, 규칙이 거꾸로 일합니다.

     그래서 **접속 인원**이 문턱을 넘으면 공유 중이어도 스스로 꺼집니다.
       · 갑자기 끄지 않습니다 — 넘은 채로 GRACE_MS(1분) 이어질 때만.
         인원은 출렁여요. 14명↔13명을 오갈 때마다 껐다 켰다 하면 최악입니다.
       · 끄기 전에 **토스트로 먼저 알립니다.** 예고 없이 꺼지면 고장으로
         읽혀요 (그게 원래 '안 끊는다'를 고른 까닭이었으니, 예고가 그 값을
         대신합니다).
       · 1분 안에 인원이 내려가면 조용히 없던 일이 됩니다.

     ★ **동시 공유 수(maxShare)로는 끄지 않습니다** (콩이 콕 집음).
       다섯이 켜 둔 것은 그 자체로 정상이에요 — 여섯째를 막으면 충분합니다.
       끄는 방아쇠는 **접속 인원 하나뿐**입니다.
     ===================================================================== */
  ...
  /** 지금 켜도 되나 — 안 되면 팝업을 띄우고 false
      ★ **동기**입니다. 기다리는 일(제한보장)은 부르는 쪽에서 미리 끝내요 —
        아래 startScreenShare 의 🍎 사파리 주석을 보세요. */
  function 켜도되나() {
    제한듣기();
  ...
  /* =====================================================================
     🔻 공유 중 자동 끄기 (2026-09-21 — 콩)
     ---------------------------------------------------------------------
     tickShare(1초)가 부릅니다. 세는 일은 순수 산수라(메모리의 _statusCache)
     서버에 한 번도 안 묻습니다.
     ===================================================================== */
  const SHARE_OVER_GRACE_MS = 60 * 1000;   // 이만큼 이어서 넘어 있을 때만 끕니다
  let _넘은때  = 0;      // 문턱을 넘기 시작한 시각 (0 이면 안 넘음)
  let _예고함  = false;  // 이번 넘김에 토스트를 띄웠나

  function 자동끄기살피기() {
    if (!_sharing) { _넘은때 = 0; _예고함 = false; return; }
    if (!_제한읽음 || !_제한.on || 방장인가()) { _넘은때 = 0; _예고함 = false; return; }

    const 접속 = 지금접속수();
    const 넘음 = _제한.max > 0 && 접속 >= _제한.max;

    if (!넘음) {
      /* 1분 안에 내려갔습니다 — 조용히 없던 일로. 다시 한산해졌다고
         또 토스트를 띄우면 그게 더 시끄러워요. */
      _넘은때 = 0; _예고함 = false;
      return;
    }
    ...
    if (!_예고함) {
      _예고함 = true;
      const 초 = Math.round(SHARE_OVER_GRACE_MS / 1000);
      try {
        window.showCommandToast?.(
          `🖥️ 지금 ${접속}명 접속 중이에요 — 안정적인 접속을 위해 ${초}초 뒤 화면 공유가 꺼져요 🙏`);
      } catch (e) {}
      return;
    }

    if (t - _넘은때 < SHARE_OVER_GRACE_MS) return;

    /* 시간이 다 됐고 여전히 넘어 있습니다 — 끕니다 */
    _넘은때 = 0; _예고함 = false;
    stopScreenShare();
    막힘팝업("자동", 접속, _제한.max);
  }
  ...
  function 막힘팝업(어느쪽, 값, 문턱) {
    ...
    const 글 = {
      접속: {
        html: `<p>지금 방에 <b>${값}명</b>이나 계셔요 🙂</p>
               <p>안정적인 접속을 위해 <b>${문턱}명</b> 이상 접속 중에는
                  화면 공유가 어려워요.</p>
               <p class="share-block-tail">조금 한산해지면 다시 켜 주세요!</p>`,
        plain: `지금 방에 ${값}명이나 계셔요 🙂\n안정적인 접속을 위해 ${문턱}명 이상 접속 중에는 화면 공유가 어려워요.\n조금 한산해지면 다시 켜 주세요!`
      },
      공유: {
        html: `<p>지금 <b>${값}명</b>이 화면을 나누고 있어요 🙂</p>
               <p>안정적인 접속을 위해 화면 공유는 <b>${문턱}명</b>까지만
                  함께 켤 수 있어요.</p>
               <p class="share-block-tail">한 자리 나면 다시 눌러 주세요!</p>`,
        plain: `지금 ${값}명이 화면을 나누고 있어요 🙂\n안정적인 접속을 위해 화면 공유는 ${문턱}명까지만 함께 켤 수 있어요.\n한 자리 나면 다시 눌러 주세요!`
      },
      /* 🔻 공유 중에 인원이 불어나 스스로 꺼진 경우 (2026-09-21).
         ★ "꺼졌다" 를 먼저 말합니다 — 무슨 일이 일어났는지가 먼저고,
           까닭은 그 다음이에요. 고장이 아니라는 것도 분명히. */
      자동: {
        html: `<p>방에 <b>${값}명</b>이 모여서<br>화면 공유를 <b>잠시 멈췄어요</b> 🙂</p>
               <p>안정적인 접속을 위해 <b>${문턱}명</b> 이상일 때는
                  공유를 쉬어 두고 있어요.</p>
               <p class="share-block-tail">한산해지면 다시 켜 주세요!</p>`,
        plain: `방에 ${값}명이 모여서 화면 공유를 잠시 멈췄어요 🙂\n안정적인 접속을 위해 ${문턱}명 이상일 때는 공유를 쉬어 둡니다.\n한산해지면 다시 켜 주세요!`
      }
    }[어느쪽];
    if (!box || !body) { alert("🖥️ 화면 공유\n\n" + 글.plain); return; }   // 껍데기가 없으면
    body.innerHTML = 글.html;
  ...
  async function startScreenShare() {
    ...
    /* 🚦 사람이 많으면 여기서 멈춥니다 — **창 고르기 판이 뜨기 전**입니다.
       ...
       🍎 [사파리 — 기다리는 자리를 여기 둔 까닭]
       getDisplayMedia(창 고르기)는 **누른 직후**에 불러야 합니다. 사이에
       기다리는 일이 끼면 브라우저가 "사람이 누른 것" 으로 안 쳐 주고,
       사파리가 특히 깐깐해요. 그래서:
         · 설정을 **이미 읽어 뒀으면 한 번도 안 기다립니다** (거의 늘 이 길)
         · 아직 못 읽었을 때만 딱 한 번 기다립니다 (40바이트, 눈 깜짝할 새)
       판단 자체(켜도되나)는 순수 산수라 기다림이 없습니다. */
    if (!_제한읽음) await 제한보장();
    if (!켜도되나()) return;
  ...
  /** 입장한 뒤에 한 번 — 아까 공유 중이었으면 단추를 깜빡입니다
      ⚠️ [2026-09-21에 알게 됨] window.offerShareResume 을 **부르는 곳이
         없습니다.** 만들어만 두고 아직 안 이어 붙인 자리예요. 그래서 여기에
         무언가를 얹으면 헛돕니다 — 실제로 🚦 제한듣기() 를 여기 얹었다가
         안 도는 걸 보고 renderShareButton 으로 옮겼습니다. */
  ...
  function renderShareButton() {
    ...
    /* 🚦 [고침 2026-09-21] 문턱 듣기를 **여기서** 붙입니다.
       판을 열 때(installShareHooks)는 아직 **로그인 전**이라 못 붙을 수 있어요.
       이 함수는 접속자 정보가 바뀔 때마다 도니 입장 직후에 반드시 한 번은
       돕니다. 이미 붙었으면 첫 줄에서 바로 돌아가므로 값이 싸요. */
    제한듣기();
  ...
  function tickShare() {
    ...
    /* 🔻 사람이 불어나면 공유를 스스로 멈춥니다 (2026-09-21 — 콩).
       ★ 여기 둔 까닭: 이 타이머는 **내가 공유 중일 때만** 돕니다. 따로
         타이머를 더 두면 공유를 안 할 때도 1초마다 헛도니까요.
       ★ 혼자 방에서도 돕니다 (아래 SOLO 반환보다 먼저) — 시험해 볼 수 있게. */
    자동끄기살피기();
