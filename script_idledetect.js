...
(function () {
  ...
  /* ═══════════════════════════════════════════════════════════════
     🪦 [되돌림 2026-08-30 — 콩] multiT 만 40분이던 것을 **20분으로** 돌립니다.
     ───────────────────────────────────────────────────────────────
     [왜 늘렸었나 · 2026-08-23]
     multiT 는 본업을 하다 짬짬이 쓰는 상태라 손이 이 화면에 안 오는
     시간이 길고, 다른 상태와 똑같이 20분에 내리면 자꾸 떨어져 제대로
     쌓이질 않는다 — 그래서 20분을 한 번 더 기다렸습니다.

     [왜 되돌리나]
     콩: "내가 해보니 악용되더라구. 어웨이 기준은 모두 동일하게 해줘."
     multiT 로 두면 40분을 자리를 비워도 작업 시간이 계속 쌓입니다.
     70% 라고는 해도 40분이면 28분이 거저 들어와요. 상태 하나만 문턱이
     두 배라는 것은, 그 상태를 고를 이유를 **일하는 방식이 아니라
     느슨함**으로 만들어 버립니다.

     ★ 되살릴 일이 생기면: 아래 _onIdle 의 `cur === "multi"` 갈래와
       MULTI_EXTRA_MS 상수를 되돌리면 됩니다. 다만 되살리기 전에
       "그 상태가 왜 필요한가" 를 먼저 물어 주세요.
     ★ 이 모든 건 **자동감지를 켠 사람에게만** 해당합니다 (콩).
     ═══════════════════════════════════════════════════════════════ */
  ...
  function _demoteToAway() {
    ...
    /* 📓 multiT 도 이제 다른 상태와 똑같이 20분입니다 (위 무덤 주석 참고) */
