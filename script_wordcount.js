...
(function () {
  ...
  function render() {
    ...
    if (_tab === "me") {
      ...
      const 랭킹 = Object.entries(sumWeek()).filter(([, v]) => v > 0);
      const 내등수 = 랭킹.sort((a, b) => b[1] - a[1]).findIndex(([n]) => n === me());

      big.textContent  = 내등수 >= 0 ? String(내등수 + 1) : "–";
      unit.textContent = `위 · 이번 주 글자수 랭킹 · 전체 ${랭킹.length}명 중`;
      rows.innerHTML = weekRankHtml();

      /* 내 줄로 스스로 스크롤 — 41줄이 될 수도 있는 목록에서 내 순위를
         찾으려고 눈으로 세지 않게 합니다 (.wc-rows 는 이미 overflow:auto). */
      const 내줄 = rows.querySelector(".wc-row.me");
      if (내줄) rows.scrollTop = Math.max(0, 내줄.offsetTop - rows.clientHeight / 2 + 내줄.clientHeight / 2);
    } else {
  ...
  /* =====================================================================
     🏆 랭킹 — 이번 주 글자수, 전체 등수 (2026-08-26 — 콩)
     ---------------------------------------------------------------------
     이 방은 줄 세우지 않는 게 취지라, 오늘 탭도 채팅처럼 흐르게 만들었을
     정도입니다(파일 맨 위 [순위를 보여주지 않는 이유] 참고). 그런데 "내가
     잘 하고 있나(꼴찌인지 중간인지 1등인지)" 를 알고 싶다는 건의가 있었고,
     콩이 고른 절충안은 —

       ① 등수는 **전체(1위~꼴찌)** 다 매깁니다 — 몇 명 중 몇 등인지가
          중요하지, 일부만 보여주면 그걸 알 수 없습니다.
       ② 그래도 **이름은 내 줄만** 보여줍니다. 남의 줄은 등수와 글자수만 —
          "누가 몇 등" 이 아니라 "내가 몇 등" 만 답하는 것으로 취지를 지킵니다.

     ★ 새로 읽는 자료가 없습니다 — sumWeek() 이 이미 붙어 있는 _week
       (이번 주 wordlog, 방 전체) 를 그대로 더합니다.
     ★ (전체 n명 중) 의 n 은 방 전체 인원이 아니라, 이번 주 글자수를
       한 번이라도 올린 사람 수입니다 — 콩이 그렇게 요청했습니다.
     ★ 목록이 길어지면(최대 41줄) .wc-rows 가 이미 scroll 을 겁니다 —
       내 줄로 자동 스크롤해서 41줄을 눈으로 세지 않게 합니다. */
  function weekRankHtml() {
    ...
    if (!랭킹.length) {
      return `<div class="wc-empty">이번 주에 아직 글자수를 올린 사람이 없어요.</div>`;
    }
    return 랭킹.map(([n, v], i) => {
      const 내것 = n === 나;
