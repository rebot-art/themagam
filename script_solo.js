...
(function () {
  ...
  /* =====================================================================
     ...
     [무엇을 하나]
       ...
       · .card-body 는 화면에서만 뒤집혀(row-reverse) 쌓기 칸이 왼쪽,
         ...
         건드립니다).
       · 프사 크기는 이제 **쌓기 칸의 키**에서 거꾸로 계산합니다
         (styles.css 의 aspect-ratio 방향을 뒤집음) — 그래서 못박아
         둘 필요가 없고, 세 줄이 쌓인 만큼 스스로 조화로운 크기에
         자리잡습니다.
       · 이름·목표 상자(.card-foot)는 못박지 않고 카드 폭을 그대로
         따라갑니다 — 늘어난 가로만큼 넓어집니다.

     [왜 두 번 재나] .card-body 는 이제 width:fit-content 라 내용
     (프사+쌓기 칸)만큼만 차지하는데, 이름·목표 상자는 폭을 따라가서
     카드가 넓어지면 목표 글자가 덜 접혀(줄바꿈이 줄어) 세로가 살짝
     바뀔 수 있습니다. 그래서 한 번 재서 넓힌 뒤, 그 폭에서 다시 한
     번 재 어긋나면 맞춥니다.

     [왜 폭도 같이 재나] .card-body 가 더는 좁게 못박혀 있지 않고
     내용만큼 자라기 때문에(프사가 커지면 .card-body 도 넓어짐), 세로
     길이만 보고 정사각형 한 변을 정하면 위 칸(프사+쌓기)이 그보다
     넓어서 삐져나올 수 있습니다. 높이와 .card-body 실제 너비 중
     **더 큰 쪽**을 씁니다.
     ===================================================================== */
  ...
  /** 지금 켜져 있으면, 작업시간을 옮긴 뒤 (카드 높이, .card-body 실제
      너비) 중 더 큰 값을 재서 정사각형 한 변으로 박습니다(두 번).
      꺼져 있으면 작업시간을 되돌리고 끝냅니다. */
  function 정사각적용() {
    const on = 정사각켬();
    ...
    /* 가장 큰 값을 씁니다 — 카드마다 목표 글자 길이가 달라 높이가
       제각각인데, 정사각형은 하나의 값을 공유해야 하니(같은 격자
       칸이라) 제일 큰 값을 써야 어떤 카드도 안 잘리거나 안 삐져나옵니다. */
    const 재기 = () => {
      let 최대 = 0;
      카드들().forEach(c => {
