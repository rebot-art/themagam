/* TheMagam © 그링링 · 무단 복제·재배포 금지 */
  ...
  function renderUserCards(data) {
      ...
      window.applyCardShape?.();    // ★ 🃏 카드 모양(세로/가로, 2026-09-07) — 좁아졌다 넓어진 사이가 있어도 한 번 더 맞춥니다
  }
  ...
  function _paintPomoPill() {
    ...
    if (_pomo && 내려둠) {
      ...
      /* [2026-09-10 — 콩] 이름을 지우지 않고 **뒤에 붙입니다.**
         「🍅 집중 · 24:59」로만 두면 이게 어느 알약이었는지가 사라져서,
         줄이 길어져도 「Pomodoro」를 남겨 둡니다. 앞 이모지만 단계에
         따라 갈아 끼워요 (🍅 집중 / ☕ 휴식 / ⏸️ 멈춤).
         ★ "Pomodoro" 는 위 DOCK 의 이름표(script_dock.js: label)와 같은
           말이어야 합니다. 저기를 바꾸면 여기도 같이 바꿔 주세요 —
           dataset.orig 에서 잘라 쓰는 방법도 있지만, 이모지를 갈아
           끼우느라 글자를 자르기 시작하면 읽기가 훨씬 어려워집니다. */
      if (라벨) 라벨.textContent =
        (_isPaused() ? "⏸️ Pomodoro · 멈춤 "
         : _pomo.phase === "rest" ? "☕ Pomodoro · 휴식 "
                                  : "🍅 Pomodoro · 집중 ")
        + mm + ":" + ss;
