...
(function () {
  ...
  async function loadAttendance(monthOffset, opts) {
    ...
    try {
      ...
      /* =====================================================================
         🏖️🌿 휴가·개인사정은 **언제나 읽습니다** (표에 바로 그려지는 값)
         ---------------------------------------------------------------------
         ★★★ [사고 2026-09-22] 그래프를 접으면서 이 줄을 `무거운것` 으로
           감쌌는데, 그 안에 휴가·개인사정 읽기까지 **같이 들어 있었습니다.**
           접힌 채로 열면 찍어 둔 장기 휴가가 표에서 통째로 사라졌어요.
           접으려던 것은 timeSegs 하나뿐이었는데, 묶여 있던 걸 못 봤습니다.
         ★ 그래서 고리를 **둘로 갈랐습니다** — 표에 그려지는 값(휴가·개인사정)은
           늘 읽고, 그래프에만 쓰는 값(timeSegs)만 접습니다.
         ★ [2026-09-21] 둘 다 **그 달만** 읽습니다.
         ===================================================================== */
      await Promise.all(nicks.map(async n => {
        try {
      ...
      /* 📊 [2026-09-22] 여기부터가 **펼칠 때만** 읽는 값입니다 (그래프·배지).
         멤버마다 그 달 구간(timeSegs)을 하나씩 — 쉰두 번이에요. */
  ...
  async function 그리기(자, 달) {
    ...
    try {
      ...
      /* =====================================================================
         ...
         ===================================================================== */
      /* ★★★ [고침 2026-09-23 — 콩 "그래프 열기 눌러도 안 닫혀"]
         예전에는 `무거운것`(= 자료를 받았나) 하나로 펼침을 정했습니다.
         그래서 ⏳ 옛 날짜 채우기처럼 **자료만 필요한** 일이 그래프까지
         펼쳐 버렸고, 한 번 펼치면 접을 길이 없었어요.
         이제 둘을 가릅니다 —
             무거운것      … 자료를 쥐고 있는가 (읽었나)
             _펼친달       … 보여 달라고 했는가 (콩의 뜻)
         보여주는 것은 **둘 다** 참일 때뿐입니다. 자료만 받아 두고 접어 두는
         일이 되니, 다시 펼칠 때 서버를 한 번도 안 읽어요. */
      const 펼침 = !!무거운것 && _펼친달.has(ymKey);
      const 접기칸 = el("adm-charts-wrap");
      const 펼침단추 = el("adm-charts-btn");
      const 접기단추 = el("adm-charts-fold-btn");
      if (접기칸) 접기칸.hidden = !펼침;
      if (펼침단추) 펼침단추.hidden = 펼침;
      if (접기단추) 접기단추.hidden = !펼침;
      ...
      const 범례 = `<p class="adm-att-legend">
        ...
        <br>${무거운것
          ? "옛 날짜는 <b>원본 구간</b>으로 가려 보여 줍니다."
          : "옛 날짜 중 <b>머문 시간이 안 적힌 날</b>은 지금 안 가려져요 — 그 값은 원본 구간에 있는데 접혀 있거든요."}
        <b>[⏳ 옛 날짜 채우기]</b> 를 그 달에서 <b>한 번</b> 눌러 두면 <b>영영</b> 해결됩니다 —
  ...
  /* =====================================================================
     ...
     pomoSessions)와 📚(worklog, 방장만 읽힘)뿐.
       ...
       ⏱ 장인    작업 시간 상위 5 (2026-09-08: 3 → 5, 다른 항목과 같게)
       ✍️ 다작    글자수 상위 5          (콩: "3명은 아쉬워")
       🍅 뽀모왕  뽀모 완주 상위 5
       📚 완결러  Work Log 회차 마침 15회 이상 (2026-09-08: 18 → 15, "하루 1빡은 빡세")
  ...
  async function fillOldStayMins() {
    ...
    let 거리 = _채울거리;
    /* 📊 [2026-09-22] 채울 재료(원본 구간)는 그래프를 접으면 안 읽습니다.
       접힌 채로 눌러도 되게, 여기서 **한 번 불러온 뒤** 채웁니다.
       ★ [고침 2026-09-23] 예전엔 여기서 _펼친달 에 넣어 버려, 채우기만
         눌렀는데 그래프가 벌컥 펼쳐졌습니다. 이제 `무거운것` 으로 자료만
         부르고 펼치지는 않아요 — 펼칠지 말지는 콩이 단추로 정합니다.
         받아 둔 값은 쥐고 있으니, 이어서 [📊 그래프 보기] 를 눌러도 공짜예요. */
    if (!거리 || !거리.minsByNick || !Object.keys(거리.minsByNick).length) {
      msg("adm-att-fill-msg", "원본 기록을 불러오는 중…");
      await loadAttendance(_attOffset, { 무거운것: true, 다시읽기: true });
      거리 = _채울거리;
    }
  ...
  /* =====================================================================
     ...
     ★ [고침 2026-09-21 — 콩] max 는 **켤 때만이 아니라 공유 중에도** 봅니다.
       "이미 켠 사람은 안 끊는다" 로 두면 한산할 때 미리 켜 두는 것이 이득이
       되어, 막으려던 바로 그 시간대가 제일 무거워집니다. maxShare 는
       그대로 켤 때만 봅니다 (다섯이 켜 둔 것은 정상이니까).
     ===================================================================== */
  ...
  async function saveShareLimit() {
    ...
    try {
      ...
      msg("adm-share-msg", on
        ? `저장했어요 — 접속 ${max}명 이상이면 못 켜고(공유 중이면 1분 뒤 꺼져요), 동시 공유는 ${cap}명까지예요. 지금 방에 있는 분들에게 바로 반영됩니다.`
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    /* 📊 그래프 접기 (2026-09-23) — 받아 둔 값은 쥔 채로 감추기만 합니다.
       그래서 서버 요청 0 이고, 다시 펼쳐도 0 이에요. */
    el("adm-charts-fold-btn")?.addEventListener("click", () => {
      _펼친달.delete(_ymKeyOf(_attOffset));
